...
for (f in files) {
  ...
  # Compute log-expression-weighted z_delta
  df$baseMean_avg <- (df$baseMean_pI + df$baseMean_cI) / 2
  df$log_weight <- log2(1 + df$baseMean_avg) / log2(1 + max(df$baseMean_avg))
  df[[metric]] <- df$z_delta * df$log_weight
...
# -------------------------------
# Clean and scale matrix  (CHANGED)
# -------------------------------
cm <- colMeans(group_matrix, na.rm = TRUE)
cs <- apply(group_matrix, 2, sd, na.rm = TRUE)
cs[!is.finite(cs) | cs == 0] <- 1  # guard

group_matrix_scaled <- sweep(sweep(group_matrix, 2, cm, "-"), 2, cs, "/")
# NOTE: keep NA; do NOT impute to 0

# -------------------------------
# PCA for UMAP denoising (CHANGED)
# -------------------------------

# Pairwise-complete cosine distance that ignores NA per pair
...
message("✅ UMAP and heatmap saved using z_LogExp_Weighted metric")
