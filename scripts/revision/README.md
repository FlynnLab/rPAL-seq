...
Sequencing-derived inputs are **not** duplicated here — they come from the GEO record (see the paper's Data
availability statement) or from the upstream steps in `scripts/workflow/` and `scripts/analysis/`:
